...
if ( ! function_exists( 'adventurous_content_nav' ) ) :
/**
...
endif; // adventurous_content_nav


if ( ! function_exists( 'adventurous_content_query_nav' ) ) :
/**
...
endif; // adventurous_content_query_nav
...
function adventurous_post_format_meta() {
	...
	// Translators: 1 is category, 2 is tag, 3 is the date and 4 is the author's name.
	if ( $tag_list ) {
		$utility_text = __( '<span class="on-date">Posted on %3$s</span><span class="in-category"> under %1$s</span><span class="sep"> | </span><span class="in-tag">Tagged %2$s</span><span class="sep"> | </span><span class="by-author"> By %4$s</span>', 'adventurous' );
	} elseif ( $categories_list ) {
		$utility_text = __( '<span class="on-date">Posted on %3$s</span><span class="in-category"> under in %1$s</span><span class="sep"> | </span><span class="by-author"> By %4$s</span>.', 'adventurous' );
	} else {
		$utility_text = __( '<span class="on-date">Posted on %3$s</span><span class="sep"> | </span><span class="by-author"> By %4$s</span>.', 'adventurous' );
	}
 ...
 * @since Adventurous 1.0
 */
function adventurous_categorized_blog() {
	...
	if ( '1' != $all_the_cool_cats ) {
		// This blog has more than 1 category so adventurous_categorized_blog should return true
		return true;
	} else {
		// This blog has only 1 category so adventurous_categorized_blog should return false
		return false;
	}
}

/**
 * Flush out the transients used in adventurous_categorized_blog
 *
 * @since Adventurous 1.0
 */
function adventurous_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'all_the_cool_cats' );
}
add_action( 'edit_category', 'adventurous_category_transient_flusher' );
add_action( 'save_post', 'adventurous_category_transient_flusher' );
