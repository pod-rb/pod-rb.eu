...
//				$query_urls['https://photoslibrary.googleapis.com/v1/albums/'.$attr['album_id']] = array('GET' => array());
				...
				if (!empty($attr['date_filters'])) {
					...
					foreach($date_filters as $date_filter) {
						...
						foreach ($dates as $idx => $date) {
							...
							if (!is_numeric($date_parts[0]) || $date_parts[0] > 9999 || $date_parts[0] < 0) return $this->error(sprintf($this->error_date_format, $date));
							if (!is_numeric($date_parts[1]) || $date_parts[1] > 12 || $date_parts[1] < 0) return $this->error(sprintf($this->error_date_format, $date));
							if (!is_numeric($date_parts[2]) || $date_parts[2] > 31 || $date_parts[2] < 0) return $this->error(sprintf($this->error_date_format, $date));
		...
		return $this->make_call($query_urls, $attr);
	}
	...
	private function process_response($body, $short_code, $deferred = false, $remove = array()) {
		global $photonic_google_photo_title_display, $photonic_google_photos_per_row_constraint, $photonic_google_photos_constrain_by_padding, $photonic_google_photos_constrain_by_count,
			   $photonic_google_photos_pop_per_row_constraint, $photonic_google_photos_pop_constrain_by_padding, $photonic_google_photos_pop_constrain_by_count, $photonic_google_photo_pop_title_display;
		$out = '';
		if (!empty($body)) {
...
//			if (empty($photo->mimeType) || stripos($photo->mimeType, 'image') !== 0) {
	...
	private function process_album($album, $short_code) {
		...
		$object['counter'] = $album->totalMediaItems;
