...
class Photonic_SmugMug_Processor extends Photonic_OAuth1_Processor {
	...
	function get_gallery_images($attr = array()) {
		...
		global $photonic_smug_thumb_size, $photonic_smug_main_size, $photonic_smug_tile_size, $photonic_smug_video_size, $photonic_smug_media;

		if (empty($this->api_key)) {
			return $this->error(sprintf(__("SmugMug API Key not defined. See <a href='%s'>here</a> for documentation.", 'photonic'), $this->doc_links['general']));
		}
	...
	function get_config_object($count = 200) {
		$config = array(
			'expand' => array(
	...
	function build_level_2_objects($albums, $album_filter = '', $short_code, $level = 0) {
