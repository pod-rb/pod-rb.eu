...
class Photonic_Picasa_Processor extends Photonic_OAuth2_Processor {
	...
	function get_gallery_images($attr = array()) {
		global $photonic_picasa_use_desc, $photonic_picasa_media;
		$attr = array_merge(
	 ...
	 */
/*	public function perform_back_end_authentication($provider, $photonic_picasa_refresh_token) {
		...
		if (!isset($photonic_authentication['picasa']) && !empty($photonic_picasa_refresh_token)) {
			$token = $this->get_access_token_from_refresh($photonic_picasa_refresh_token, true);
		}
