...
abstract class Photonic_Processor {
	public $library, $thumb_size, $full_size, $api_key, $api_secret, $provider, $nonce, $oauth_timestamp, $signature_parameters, $link_lightbox_title, $layout,
		...
		$doc_links, $server_error, $password_protected, $token, $token_secret, $show_buy_link;
	...
	function __construct() {
		...
		$this->server_error = sprintf(__("There was an error connecting to %s. Please try again later.", 'photonic'), $this->provider);
		$this->password_protected = __('This album is password-protected. Please provide a valid password.', 'photonic');
				...
				if (empty($hidden['title']) || empty($hidden['counter']) || empty($iterate_level_3)) {
					...
					if (empty($hidden['counter'])) {
						...
						if (!empty($counters['videos'])) {
							$counter_texts[] = sprintf(_n('%s video', '%s videos', $counters['videos'], 'photonic'), $counters['videos']);
						}
	...
	function wp_error_message($response, $server_msg = true) {
		$ret = '';
		if ($server_msg) {
			$ret = $this->server_error."<br/>\n";
		}
		if (is_wp_error($response)) {
			$messages = $response->get_error_messages();
			$ret .= sprintf(_n('<strong>%s Message:</strong>', '<strong>%s Messages:</strong>', count($messages), 'photonic'), count($messages))."<br/>\n";
