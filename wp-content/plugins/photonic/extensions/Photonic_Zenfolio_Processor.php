...
class Photonic_Zenfolio_Processor extends Photonic_Processor {
	...
	public function get_gallery_images($attr = array()) {
		global $photonic_zenfolio_thumb_size, $photonic_zenfolio_main_size, $photonic_zenfolio_tile_size, $photonic_zenfolio_title_caption, $photonic_zenfolio_video_size, $photonic_zenfolio_media;
			...
			switch ($attr['view']) {
				...
				case 'photosets':
					if (!empty($object_id)) {
						...
						return $this->error(sprintf(__('The <code>photoset_type</code> parameter is required if <code>object_id</code> is not specified. See <a href="%s">here</a> for documentation.', 'photonic'), $this->doc_links['photosets']));
					}
					break;
...
//		$force_secure = false;
	...
	function process_group_hierarchy($response, $options = array(), $short_code = array()) {
		...
		$ret = $this->process_group($response, $options, $short_code, 0);
		return $ret;
	}
	 ...
	 * @return string
	 */
	function process_group($group, $options = array(), $short_code = array(), $level, &$all_photosets = array()) {
		$ret = '';
		$type = '$type';
		...
		$all_photosets = array_merge($all_photosets, $photosets);
		...
		foreach ($groups as $group) {
			$out = $this->process_group($group, $options, $short_code, $level + 1, $all_photosets);
			if ($short_code['structure'] !== 'flat') {
				$ret .= $out;
			}
		}
