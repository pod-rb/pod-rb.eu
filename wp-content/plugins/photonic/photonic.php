...
class Photonic {
	var $version, $registered_extensions, $defaults, $plugin_name, $options_page_name, $settings_page, $helper_page, $getting_started_page, $authentication_page;
	function __construct() {
		global $photonic_options, $photonic_setup_options, $photonic_is_IE;
		if (!defined('PHOTONIC_VERSION')) {
			define('PHOTONIC_VERSION', '1.68');
		}
...
//		add_action('wp_ajax_photonic_flow_next_screen', array(&$this, 'flow_next_screen'));
	...
	function add_admin_menu() {
		...
		$this->options_page_name = add_menu_page('Photonic', 'Photonic', 'edit_posts', 'photonic-options-manager', array(&$photonic_options_manager, 'render_settings_page'), plugins_url('include/images/Photonic-16.png', __FILE__));
	...
	function add_admin_scripts($hook) {
		if ($this->options_page_name == $hook) {
			...
			wp_enqueue_script('photonic-admin-js', plugins_url('include/scripts/admin.js', __FILE__), array('jquery', 'wp-color-picker'), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/admin.js'));
			wp_localize_script('photonic-admin-js', 'Photonic_Admin_JS', $js_array);
		}
		else if ($this->helper_page == $hook || $this->authentication_page == $hook) {
			...
			if (empty($photonic_disable_editor)) {
				...
				wp_enqueue_script('photonic-admin-js', plugins_url('include/scripts/gallery-settings.js', __FILE__), array('jquery', 'media-views'), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/gallery-settings.js'));
				wp_localize_script('photonic-admin-js', 'Photonic_Admin_JS', $js_array);
				wp_enqueue_style('photonic-upload', plugins_url('include/css/admin-form.css', __FILE__), array(), $this->get_version(plugin_dir_path(__FILE__).'include/css/admin-form.css'));
	...
	function add_scripts() {
		...
		wp_enqueue_script('photonic-slideshow', plugins_url('include/scripts/lightslider/lightslider.min.js', __FILE__), array('jquery'), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/lightslider/lightslider.min.js'));

		if ($photonic_is_IE) {
			wp_enqueue_script('photonic-masonry', plugins_url('include/scripts/masonry/masonry.pkgd.min.js', __FILE__), array('jquery'), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/masonry/masonry.pkgd.min.js'));
			wp_enqueue_script('photonic-ie', plugins_url('include/scripts/photonic-ie.js', __FILE__), array('photonic'), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/photonic-ie.js'));
		}
		...
		if ($photonic_slideshow_library == 'thickbox') {
			...
			wp_enqueue_script('photonic-lightbox', plugins_url('include/scripts/'.$photonic_slideshow_library.'/'.$photonic_slideshow_library.'.min.js', __FILE__), array('jquery'), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/'.$photonic_slideshow_library.'/'.$photonic_slideshow_library.'.min.js'));
			if ($photonic_slideshow_library == 'lightgallery') {
				...
				foreach ($lightgallery_plugins as $plugin) {
					wp_enqueue_script('photonic-lightbox-'.$plugin, plugins_url('include/scripts/'.$photonic_slideshow_library.'/lg-plugin-'.$plugin.'.min.js', __FILE__), array('photonic-lightbox'), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/'.$photonic_slideshow_library.'/lg-plugin-'.$plugin.'.min.js'));
				}
			}
		...
		wp_enqueue_style("photonic-slideshow", plugins_url('include/scripts/lightslider/css/lightslider.css', __FILE__), array(), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/lightslider/css/lightslider.css'));

		if ($photonic_slideshow_library == 'fancybox') {
			wp_enqueue_style("photonic-lightbox", plugins_url('include/scripts/fancybox/jquery.fancybox-1.3.4.css', __FILE__), array(), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/fancybox/jquery.fancybox-1.3.4.css'));
		}
			...
			if ($photonic_cbox_theme == 'theme' && @file_exists($stylesheet_directory.'/scripts/colorbox/colorbox.css')) {
				...
				wp_enqueue_style("photonic-lightbox", plugins_url('include/scripts/colorbox/style-1/colorbox.css', __FILE__), array(), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/colorbox/style-1/colorbox.css'));
			}
			else {
				wp_enqueue_style("photonic-lightbox", plugins_url('include/scripts/colorbox/style-'.$photonic_cbox_theme.'/colorbox.css', __FILE__), array(), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/colorbox/style-'.$photonic_cbox_theme.'/colorbox.css'));
			}
		}
		else if ($photonic_slideshow_library == 'prettyphoto') {
			wp_enqueue_style("photonic-lightbox", plugins_url('include/scripts/prettyphoto/css/prettyPhoto.css', __FILE__), array(), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/prettyphoto/css/prettyPhoto.css'));
		}
		else if ($photonic_slideshow_library == 'swipebox') {
			wp_enqueue_style("photonic-lightbox", plugins_url('include/scripts/swipebox/css/swipebox.min.css', __FILE__), array(), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/swipebox/css/swipebox.min.css'));
		}
		else if ($photonic_slideshow_library == 'imagelightbox') {
			wp_enqueue_style("photonic-lightbox", plugins_url('include/scripts/imagelightbox/imagelightbox.css', __FILE__), array(), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/imagelightbox/imagelightbox.css'));
		}
		else if ($photonic_slideshow_library == 'lightcase') {
			wp_enqueue_style("photonic-lightbox", plugins_url('include/scripts/lightcase/lightcase.css', __FILE__), array(), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/lightcase/lightcase.css'));
		}
		else if ($photonic_slideshow_library == 'lightgallery') {
			wp_enqueue_style("photonic-lightbox", plugins_url('include/scripts/lightgallery/css/lightgallery.min.css', __FILE__), array(), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/lightgallery/css/lightgallery.min.css'));
		}
		else if ($photonic_slideshow_library == 'featherlight') {
			wp_enqueue_style("photonic-lightbox", plugins_url('include/scripts/featherlight/css/featherlight.min.css', __FILE__), array(), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/featherlight/css/featherlight.min.css'));
		}
		else if ($photonic_slideshow_library == 'fancybox3') {
			wp_enqueue_style("photonic-lightbox", plugins_url('include/scripts/fancybox3/fancybox3.min.css', __FILE__), array(), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/fancybox3/fancybox3.min.css'));
		}
	...
	function admin_init() {
		if (isset($_REQUEST['page']) && ('photonic-options-manager' == $_REQUEST['page'] || 'photonic-options' == $_REQUEST['page'] || 'photonic-helpers' == $_REQUEST['page'] || 'photonic-getting-started' == $_REQUEST['page']|| 'photonic-auth' == $_REQUEST['page'])) {
			global $photonic_options_manager;
	...
	function modify_gallery($content, $attr = array()) {
		global $post, $photonic_flickr_gallery, $photonic_picasa_gallery, $photonic_google_gallery, $photonic_native_gallery, $photonic_500px_gallery,
		...
		switch ($type) {
			...
			case 'google':
				...
				$images = $photonic_google_gallery->get_gallery_images($attr);
				break;
	...
	function obtain_token() {
		global $photonic_picasa_gallery, $photonic_google_gallery, $photonic_flickr_gallery, $photonic_smugmug_gallery, $photonic_500px_gallery;
		$provider = sanitize_text_field($_POST['provider']);
		if ($provider == 'picasa') {
	...
	public function get_localized_js_variables() {
		global $photonic_fbox_title_position, $photonic_slideshow_library, $photonic_custom_lightbox,
			   ...
			   $photonic_enable_swipebox_mobile_bars, $photonic_lightbox_for_all, $photonic_lightbox_for_videos, $photonic_popup_panel_width, $photonic_deep_linking, $photonic_social_media,
			   ...
			   $photonic_enable_fb3_fullscreen, $photonic_enable_fb3_thumbnail;
	...
	function add_photonic_button() {
		...
		printf('<a href="%s" class="button photonic-button thickbox" id="photonic-add-gallery" title="Photonic Gallery"><img class="wp-media-buttons-icon" src="'.plugins_url('include/images/Photonic-16.png', __FILE__).'"/> %s</a>',
			$url, __( 'Add Photonic Gallery', 'photonic')); // admin_url('admin.php?action=photonic_flow&height=600&width=1000&TB_iframe=true')
	}

	function insertion_flow() {
		define( 'IFRAME_REQUEST', true );
		$this->enqueue_flow_scripts();
		iframe_header('Add Photonic Gallery');
		require_once(plugin_dir_path(__FILE__).'/admin/flow.php');
		iframe_footer();
		exit;
	}

	function enqueue_flow_scripts() {
		global $photonic_flickr_api_key, $photonic_flickr_api_secret, $photonic_picasa_client_id, $photonic_picasa_client_secret, $photonic_picasa_refresh_token;
		$flow_js = array(
			'flickr_api_key' => $photonic_flickr_api_key,
			'flickr_api_secret' => $photonic_flickr_api_secret,
			'picasa_client_id' => $photonic_picasa_client_id,
			'picasa_client_secret' => $photonic_picasa_client_secret,
			'picasa_refresh_token' => $photonic_picasa_refresh_token,
		);
		...
		wp_enqueue_script('photonic-flow-js', plugins_url('include/scripts/flow.js', __FILE__), array('jquery'), $this->get_version(plugin_dir_path(__FILE__).'include/scripts/flow.js'));
		wp_localize_script('photonic-flow-js', 'Photonic_Flow_JS', $flow_js);
	}

	function flow_next_screen() {
		require_once(plugin_dir_path(__FILE__)."/admin/flow-manager.php");
		if (isset($_POST['provider'])) {
			$flow_manager = new Photonic_Flow_Manager(plugin_dir_path(__FILE__));
