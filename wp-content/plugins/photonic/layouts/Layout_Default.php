...
class Photonic_Layout {
	...
	function generate_level_1_gallery($photos, $options, $short_code, $processor) {
		...
		$start_with = "$indent<$container_tag $ul_class $pagination_data $columns_data>\n";
		$ret .= $start_with;
...
//			$orig = $photo['main_image'];
			...
			else if (!empty($photo['video']) && (in_array($this->library, array('colorbox', 'fancybox', 'fancybox2', 'swipebox'))
...
//					|| (in_array($this->library, array('featherlight')) && in_array($processor->provider, array('google')))
			...
			$ret .= $indent."\t\t".'<a '.$link_attributes.' href="'.$orig.'" title="'.esc_attr($title).'" data-title="'.$title_markup.'" '.$lb_specific_data.' '.$target.$deep_link.$buy.">\n";
	...
	function generate_level_2_gallery($objects, $options, $short_code, $processor) {
		...
		$ret = "\n$indent<ul $ul_class $pagination_data $columns_data>";
		if ($non_standard) {
			$ret = "\n$indent<div $ul_class $pagination_data>";
		}
	...
	function get_lightbox_attributes($display, $col_class, $show_lightbox, $rel_id, $processor) {
		...
		if ($this->library != 'none' && $show_lightbox) {
			...
			switch ($this->library) {
				...
				case 'fancybox3':
					$class = 'photonic-launch-gallery launch-gallery-fancybox fancybox';
					$lightbox_specific_attr = ' data-fancybox="'.$rel.'" ';
					break;
				...
				case 'featherlight':
					$class = 'photonic-launch-gallery launch-gallery-featherlight';
					break;
...
//				$class .= ' '.$col_class;
	...
	function get_lightbox_specific_photo_data($photo_data, $processor) {
		...
		else if (in_array($this->library, array('colorbox', 'fancybox', 'fancybox2', 'swipebox', )) ||
